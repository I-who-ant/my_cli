...
class _ToolCallManager:
    ...
    def start_tool_call(self, tool_call: ToolCall):
        ...
        # ⚠️ 不在这里显示参数！
        # 理由：参数可能通过流式 ToolCallPart 到达，此时还不完整
        # 等待所有参数收集完毕后，在 finish_tool_call() 中显示
    ...
    def finish_tool_call(self, tool_result: ToolResult):
        """工具调用完成，显示参数和结果"""
        # 在工具调用完成时，显示完整参数
        if self._current_tool_call and self._current_arguments:
...
async def _handle_approval_request(
    ...
    panel_text.append(f"操作: ", style="grey50")
    panel_text.append(f"{request.description}\n\n", style="white")
    panel_text.append("请选择:\n", style="grey50")
    panel_text.append("  [y] 批准本次\n", style="cyan")
    panel_text.append("  [a] 批准本会话所有同类操作\n", style="cyan")
    panel_text.append("  [n] 拒绝\n", style="cyan")

    # 2. 使用 Group 组合内容和面板
    approval_panel = Panel(panel_text, title="⚠️ 批准请求", border_style="yellow")
    live.update(Group(content_text, approval_panel))
    ...
    try:
        ...
        # 4. 调用 request.resolve() 返回响应
        request.resolve(response)

    finally:
        # 5. 恢复 Live 渲染（只显示内容，不显示面板）
        live.start()
        live.update(content_text)
